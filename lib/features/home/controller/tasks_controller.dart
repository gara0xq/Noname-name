...
class TasksController extends GetxController {
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (responseData.containsKey('tasks')) {
          ...
          print(' Stats - Total: $totalCount, Pending: $pendingCount, Submitted: $submittedCount, Completed: $completedCount, Expired/Declined: $expiredDeclinedCount');
        }
      }
    } catch (e) {
      hasError.value = true;
  ...
  void _calculateStats() {
    pendingCount.value = allTasks.where((task) => 
      task.status == 'pending' && !task.isExpired
    ).length;
    
    submittedCount.value = allTasks.where((task) => 
      task.status == 'submitted'
    ).length;
    
    completedCount.value = allTasks.where((task) => 
      task.status == 'completed'
    ).length;
    
    expiredDeclinedCount.value = allTasks.where((task) => 
      task.status == 'Declined' || task.isExpired
    ).length;
